<h1 align="center">Kyreqo</h1>

<p align="center">
  Kyreqo is a modern web-based API testing platform built with React and Django REST Framework.
</p>

## Tech Stack

### Frontend
- React (Vite)
- TypeScript
- Tailwind CSS

### Backend
- Django
- Django REST Framework
- PostgreSQL

---

## 🗺️ Project Roadmap & Execution Order

To prevent blockages and maintain full integration parity between Frontend and Backend, work must be completed in the exact order below. 

Click on the issue links below to view detailed requirements, tasks, and code branches:

1. **Backend (Debasis)**: [#2](https://github.com/DebaA17/kyrego/issues/2) DRF Authentication & Custom User Accounts
2. **Frontend (Ruchika)**: [#1](https://github.com/DebaA17/kyrego/issues/1) JWT Authentication UI & Global State Context
3. **Backend (Debasis)**: [#3](https://github.com/DebaA17/kyrego/issues/3) DRF Workspaces & Workspace Membership APIs
4. **Frontend (Ruchika)**: [#4](https://github.com/DebaA17/kyrego/issues/4) Interactive Workspace Switcher Dropdown
5. **Backend (Debasis)**: [#7](https://github.com/DebaA17/kyrego/issues/7) DRF Collections & Request Persistence CRUD APIs
6. **Frontend (Ruchika)**: [#5](https://github.com/DebaA17/kyrego/issues/5) Collections Explorer & Tree View
7. **Backend (Debasis)**: [#8](https://github.com/DebaA17/kyrego/issues/8) DRF Environments Variables CRUD APIs
8. **Frontend (Ruchika)**: [#6](https://github.com/DebaA17/kyrego/issues/6) Environment Variables Manager Modal

---

## Development Status

🚧 Currently under development.

## License

[MIT License](LICENSE)

